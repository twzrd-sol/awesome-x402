...
- [TWZRD Agent Intelligence](https://intel.twzrd.xyz) - Trust + reputation intelligence for x402 agents on Solana. A free preflight ReadinessCard scores any counterparty on a wash/Sybil-resistant cross-facilitator corpus (decide *before* you pay); paid `GET /v1/intel/trust/{pubkey}` ($0.05 USDC, Solana mainnet) returns the trust score plus a portable Ed25519-signed V5 receipt verifiable offline (no trust in the server). Free score / leaderboard / receipt-verify endpoints. Indexed on x402scan; MCP server on the official MCP Registry. ([OpenAPI](https://intel.twzrd.xyz/openapi.json)) ([x402 Discovery](https://intel.twzrd.xyz/.well-known/x402)) ([MCP](https://pypi.org/project/twzrd-agent-intel/))
